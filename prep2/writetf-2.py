		...
		features=tf.train.Features(
			feature={
				...
				'vec': tf.train.Feature(
					float_list=tf.train.FloatList(value=vec.flatten().tolist())),
				'proj': tf.train.Feature(
					float_list=tf.train.FloatList(value=proj.flatten().tolist())),
...
new_dir = directory + '/' + group + '_100_100_200x' + '/'
...
for patient in labels:
	if os.path.isfile(old_dir + patient[0] + ".mat"):
		...
		if nslices >=100 and nslices<200:
			...
			if not os.path.isfile(new_dir + patient[0] + "_2.tfrecords"):
				mat = read_mat(old_dir + patient[0], mat)
				if mat:
